...
var (
	...
	ErrMethodNotAllowed         = &AppErr{HttpCode: http.StatusOK, Code: "1009", Msg: map[string]string{CHINESE: "不支持的请求方法", ENGLISH: "method not allowed"}}
...
func InitMsgLanguage(lang string) {
	LANG = lang
}
...
func ResponseOK(w http.ResponseWriter, data interface{}) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"code": "0000", "msg": msg, "data": data})
}

func ResponseErr(w http.ResponseWriter, err *AppErr, reason ...string) {
	w.WriteHeader(err.HttpCode)
	w.Header().Set("Content-Type", "application/json")
	if len(reason) > 0 {
		json.NewEncoder(w).Encode(map[string]interface{}{"code": err.Code, "msg": err.Msg[LANG], "reason": reason[0], "data": ""})
	} else {
		json.NewEncoder(w).Encode(map[string]interface{}{"code": err.Code, "msg": err.Msg[LANG], "data": ""})
	}
}
